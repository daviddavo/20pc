		...
		try {
			...
			while (connected) {
				...
				case MENSAJE_LISTA_USUARIOS:
					_oos.writeUnshared(new ListaUsuariosConfirmacionMensaje(msg.getOrigen(), Servidor.SERVIDOR, _servidor.getListaUsuarios()));
					break;
				...
				{
					// Paso 1: Obtener el socket y el usuario asignado a dicho fichero
					...
					// Paso 2: Enviar el mensaje MENSAJE_EMITIR_FICHERO al usuario
					if (oc != null)
				...
				{
					// "Reenviar" mensaje (cambiando el tipo) al cliente que lo necesite
				...
				{
					InfoFicheroMensaje ifmsg = (InfoFicheroMensaje) msg;
					_servidor.addInfoFichero(ifmsg.getOrigen(), ifmsg.getInfoFichero());
					_oos.writeUnshared(new InfoFicheroAckMensaje(Servidor.SERVIDOR, ifmsg.getOrigen()));
				}
